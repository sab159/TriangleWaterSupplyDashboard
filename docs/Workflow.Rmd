...
```{r read2}
pwsid <- registry$pwsid[1] # retrieve the pwsid of the first row

# We write a convenience function to check a
# pwsid against an endpoint variable we have already set
staThingCheck <- function(api_url,id){
  call <- paste0(api_url,"Things('",id,"')") 
  # This makes "https://example-sta-api_url.com/endpoint-interface/Things('{id}')
  response <- httr::GET(call)$status
  return(response)
}

staThingCheck(endpoint,pwsid)
...
Now we can access the metadata sheet using `data$metadata` to create the Thing and Location.
