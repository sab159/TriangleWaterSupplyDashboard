...
// RESERVOIR GAUGE HOVER ----------#############################################################################
  map.on('mousemove', 'precipitation', function(e) {
    ...
    map.getCanvas().style.cursor = 'pointer';
      var coordinates = e.features[0].geometry.coordinates.slice();
      var now_date = e.features[0].properties.date;
      var description = e.features[0].properties.name + "<br>last measured (" + now_date + ")" + 
          "<br>Network: " + e.features[0].properties.network;
  ...
  map.on('mouseleave', 'forecastPCP', function () {
   ...
   // PCP Forecast ----------#############################################################################
  map.on('mousemove', 'tempPCP', function(e) {
    map.getCanvas().style.cursor = 'pointer';
    // Populate the popup and set its coordinates based on the feature found.
    var coordinates = e.lngLat;
    var description = e.features[0].properties.Name;
  ...
  map.on('mouseleave', 'tempPCP', function () {
    map.getCanvas().style.cursor = '';
    popup.remove();
  });
...
map.on('click', 'streamgauge-layer', function (e){
  ...
  document.getElementById("selectDataName").innerHTML = "<h4>" + streamID + ": <a href=" + urlLink + " target='_blank'>" + streamGaugeName +"</a><h4>";
...
map.on('click', 'reservoirs', function (e){
  ...
  document.getElementById("selectDataName").innerHTML = "<h4>" + streamID + ": <u><a style='color: rgb(26,131,130);' href=" + urlLink + " target='_blank'>" + reservoirName +"</a></u><h4>";
...
map.on('click', 'precipitation', function (e){
  ...
  var pcpRef = "https://climate.ncsu.edu/cronos/?station=" + pcpID + "&temporal=D target='_blank'>";
  document.getElementById("pcpTitle").innerHTML = pcpID +": <a href=" + pcpRef +  precipName + "</a>";
  document.getElementById("pcpMetadata").innterHTML = "(Last reading: " +  e.features[0].properties.date + ")";
