...
d3.csv("data/pcp/pcp_months_total.csv").then(function(dfpcp){
...
d3.csv("data/pcp/pcp_cum_total.csv").then(function(cumpcp){
  ...
  var selpcp = cumpcp.filter(function(d){ return d.id === pcpID && d.date !== "Feb-29"; });
  ...
  //draw 2020 year
  var yCurrent = selpcp.filter(function(d) {return d.year === currentYear;}).map(function(d) { return d.pcp_in; });
  xJulian = selpcp.filter(function(d) {return d.year === currentYear;}).map(function(d) { return d.date; });
  var trace2020 = {
            x: xJulian,   y: yCurrent,
            mode: 'lines', type: 'scatter',
            hovertemplate: "%{y:.1f} inches by %{x} days of " + currentYear,
            opacity: 1,
            line: {color: 'black', width: 3}, 
            name: ""
          };
