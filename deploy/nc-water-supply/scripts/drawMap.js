...
//LOAD MAPS----------------------------------------------------------------
  ...
  map.on('load', function() {
    ...
    map.addSource('utilities',{
      type: 'geojson',
      //data: 'data/triangle_systems.geojson'
      data: utilities
    }); // end addSource
...
//#########################################################################################
  ...
  console.log(myUtilityID);
      ...
      type: 'geojson',
      data: 'data/water_supply_watersheds.geojson'
    });
    map.addLayer({
      'id': 'water_supply',
      'type': 'fill',
...
//#########################################################################################
 ...
 //add pcp 7 day obxervation to map------------------------------------------------
    map.addSource('pcp7obsv',{
      type: 'geojson',
      data: 'data/pcp_7day_obsv.geojson'
    }); // end addSource
    map.addLayer({
    'id': 'pcp7obsv',
    'type': 'fill',
  ...
  //add pcp 7 day obxervation to map------------------------------------------------
    map.addSource('pcp7norm',{
      type: 'geojson',
      data: 'data/pcp_7day_percent_normal.geojson'
    }); // end addSource
    map.addLayer({
    'id': 'pcp7norm',
    'type': 'fill',
    ...
    map.addSource('forecastPCP',{
      type: 'geojson',
      data: 'data/pcp610forecast.geojson'
    }); // end addSource
    map.addLayer({
    'id': 'forecastPCP',
    'type': 'fill',
    ...
    map.addSource('forecastTEMP',{
      type: 'geojson',
      data: 'data/temp610forecast.geojson'
    }); // end addSource
    map.addLayer({
    'id': 'forecastTEMP',
    'type': 'fill',
    ...
    map.addSource('qpf7day',{
      type: 'geojson',
      data: 'data/qpf1-7dayforecast.geojson'
    }); // end addSource
    map.addLayer({
    'id': 'qpf7day',
    'type': 'fill',
...
//#########################################################################################
 //STREAM GAUGES------------------------------------------------------
   map.addSource('streamgauges', {
    'type': 'geojson',
    'data': 'data/triangle_stream_gauge_sites.geojson',
    'generateId': true
   });
    map.addLayer({
   ...
   map.addSource('reservoirs', {
    ...
    'type': 'geojson',
    'data': 'data/pcp/triangle_pcp_sites.geojson',
    'generateId': true
   });
    map.addLayer({
      'id': 'precipitation',
       ...
       streamLegend.innerHTML = "<h4>Current Status</h4>";
        ...
        if (mapLayer === "pcp7obsv"){ 
          if (visibility === 'none') {
            ...
            var layers = ["0", "0.01-0.25", "0.26-0.50", "0.51-1.00", "1.01-1.50", "1.51-2.00", "2.01-3.00", "3.01-4.00", "4.01-5.00",
                          "5.01-6.00","6.01-8.00", "8.01-10.00", "10.01-20.00", ">20"];
            var colors = ["white",'#3fc1bf','#87b2c0', '#000080', "#00fc02", '#56b000', '#316400', "yellow", '#f7e08b', "orange", 'red',
                          '#9a0000', '#4e0000', '#e00079', '#8e2eff'];
        ...
        if (mapLayer === "pcp7norm"){ 
          if (visibility === 'none') {
            ...
            var colors = ["white",'#4e0000','#9a0000', 'red', "orange",'#f7e08b', 'yellow', "#eeee90", '#bcf5bc', '#00fc02',
                        "#56b000", "#316400", '#3fc1bf', '#000080', '#8e23ff', '#e00079'];
        ...
        if (mapLayer === "qpf7day"){ 
          if (visibility === 'none') {
            ...
            var layers = ["<= 0.01", "0.02-0.10", "0.11-0.25","0.26-0.50", "0.51-0.75", "0.76-1.00", "1.01-1.25", "1.26-1.50",
                          "1.51-1.75", "1.76-2.00", "2.01-2.50", "2.51-3.00", "3.01-4.00", "4.01-5.00", "5.01-7.00", "7.01-10.0",
                          "10.1-15.0", "15.1-20.0", "> 20"];
            var colors = ["lightgray", "#228b22", "#2cb42c","#000080","#000072","#005fbf", "#007cfa", "#00bfbf",
                          "#9370db", "#663399", "#800080", "darkred", "red", "#ff4500", "orange", "#8b6313", "#daa520", "yellow", "black"];
