 ...
 //add pcp 7 day obxervation to map------------------------------------------------
  ...
  //add pcp 7 day obxervation to map------------------------------------------------
    ...
    'paint': {'fill-color': [
              ...
              'fill-opacity': 0.2, //Sometimes there is a problem with file have extra layers
             }
    });
...
//#########################################################################################
 //STREAM GAUGES------------------------------------------------------
   map.addSource('streamgauges', {
    'type': 'geojson',
    'data': 'data/streamflow/stream_gauge_sites.geojson',
    'generateId': true
   });
    map.addLayer({
   ...
   map.addSource('reservoirs', {
    'type': 'geojson',
    'data': 'data/reservoirs/usace_sites.geojson',
    'generateId': true
   });
    map.addLayer({
      'id': 'reservoirs',
    ...
    'type': 'geojson',
    'data': 'data/pcp/pcp_sites.geojson',
    'generateId': true
   });
    map.addLayer({
      'id': 'precipitation',
       ...
       streamLegend.innerHTML = "<h4>Current Status</h4>";
        ...
        if (mapLayer === "pcp7norm"){ 
          if (visibility === 'none') {
            ...
            var colors = ["white",'#4e0000','#9a0000', 'red', "orange",'#f7e08b', 'yellow', "#316400", '#00fc02',
